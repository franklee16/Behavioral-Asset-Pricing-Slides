...
\item \textbf{A potential explanation for momentum} (Da, Gurun and Warachka, 2014)
  ...
  \item ``Frog in the Pan'' hypothesis: Investors are more likely inattentive to information arriving continuously in small amounts than to information that arrives in large amounts at discrete timepoints
...
\item \textbf{Hong and Li (JFQA 2019)}, ``The Information Content of Sudden Insider Silence'' 
  ...
  \item Sudden silence is a deviation from routine trading pattern
  ...
  \end{itemize}
\item \textbf{Connection:} the previous slide shows investors underreact to subtle news; here they underreact to the \emph{absence} of an expected event, because it is hard to recognizing nonoccurrence sometimes could be informative 
\end{itemize}
\end{frame}
...
\begin{frame}[shrink=5]{Your Instructor's Research: Geographic and Scope Links}
\begin{itemize}
...
\item Han, Subrahmanyam, and Zhou (JFE 2017)
  \begin{itemize}
  \item Term structure of firms' credit spreads predicts future firm fundamentals and stock returns
  \end{itemize}
\item Addoum and Murfin (RFS 2020)
  \begin{itemize}
  \item Prices of syndicated loans contain useful information for equity prices but investors fail to unravel such information, leading to return predictability from loan market to equity market
  \end{itemize}
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
...
\item They leverage the unique setting in China where the order of stocks displayed on screen is determined by the stock's listing code
...
\begin{column}{0.5\textwidth}
\begin{itemize}
\item Because attention leads to excessive buying demand, stocks displayed next to those with higher returns in the past two weeks are associated with higher returns in the future week, which revert in the long run
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\centering
...
\item \textbf{Analysts?}
  \begin{itemize}
  	\item Most studies show analyst recommendation and price targets are biased and overly optimistic 
...
\item Interestingly, actions of sophisticated investors, when aggregated to the market level, can predict market returns in the same direction as stock-level returns
  \begin{itemize}
  \item Baker and Wurgler (2000): share of equity issues in total equity and debt issues negatively predicts market return
  \item Seyhun (1988): aggregate trading by corporate insiders positively predicts future market return
  \item Rapach, Ringgenberg, and Zhou (2016): aggregate short interests negatively forecasts market return
  \end{itemize}
\end{itemize}
\vfill
\centering
...
\item \textbf{Barber, Odean, and Zhu (2008) and Hvidkjaer (2008)}
  ...
  \item Use small trades in TAQ dataset to identify individuals and the Lee-Ready algorithm to identify buy/sell orders
  \end{itemize}
...
\begin{frame}[shrink=5]{My Research: Information Acquisition via EDGAR Search}
	\begin{itemize}
		...
		\end{itemize}
		\item \textbf{Connection:} Google search volume measure what grabs retail investors' attention; EDGAR search measures deliberate, costly processing by sophisticated investors. The two behave oppositely: attention-driven demand reverses, informed acquisition does not
	\end{itemize}
\end{frame}
...
\vfill
\centering
\includegraphics[width=0.80\textwidth]{figures/BW_sentiment_index.png}
\vfill
\end{frame}
...
\item Consistent with the idea that sentiment exhibits its strongest effects on asset prices at times when valuations are most subjective
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\centering
...
\item Stambaugh, Yu, and Yuan (JFE 2012) argue that anomalies should be amplified following high sentiment
  \begin{itemize}
  \item The effect should concentrate on short-leg due to short-sale constraints
  \end{itemize}
\end{itemize}
\vfill
\centering
...
\item Birru (JFE 2018) documents that long-short anomaly returns are strongly related to the day of the week
  \begin{itemize}
  \item In particular, long-short strategies earn the highest return on Monday and the lowest return on Friday when the short-leg is more speculative
...
\begin{itemize}\small
\item FEARS is negatively related with contemporaneous returns
\item FEARS is positively related with future returns
...
\item Davies (2022) constructs a Speculative Sentiment Index (SSI) based on this insight
  ...
  \end{itemize}
\item 1-standard-deviation increase in the monthly index is associated with a 1.14\%--1.67\% decline in broad-market stocks in the next month
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\centering
...
\item Economic magnitudes are significant:
  \begin{itemize}
  \item 1-$\sigma$ increase in ISS EDF $\implies$ cumulative excess returns fall by 7.30\% over the following 2 years
  \end{itemize}
\end{itemize}
\vfill
\centering
...
\begin{frame}{Credit Expansion Predicts Banking Crisis}
\begin{columns}
\begin{column}{0.5\textwidth}
\begin{itemize}
...
\begin{frame}{Narratives of Credit Cycle}
\begin{itemize}
\item Short-term rates are low or have fallen in the recent past
  \begin{itemize}
  \item Push ``reaching for yield'' investors to buy bonds with longer duration and lower credit quality 
  \end{itemize}
\item Term and credit spread become compressed as a result 
\item Default rates have been low or falling, because lower credit spread allow low-quality firms to get cheap financing easily 
\item This credit boom incentivises low-quality firms to take excessive leverage, which become vulnerable to even small external shock 
\item Consistent with empirical evidence on boom-burst credit cycle 
\end{itemize}
\end{frame}
...
\item \textbf{Bounded rationality:}
  \begin{itemize}
  \item Limited attention leads to underreaction to news that are less salient or more costly to process  
  \item Category thinking leads to excess return comovement
  \end{itemize}
\item \textbf{Psychology-free approaches:}
  \begin{itemize}
  \item Actions of sophisticated traders predict returns correctly 
