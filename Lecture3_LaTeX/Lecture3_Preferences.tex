...
\item To derive the predictions of irrationality, we usually need to make more assumptions about the form of irrationality
  ...
  \end{itemize}
\item Rational benchmark has two elements:
  \begin{itemize}
  \item When they receive new information, agents update their beliefs correctly (according to Bayes' rule)
...
\item \textbf{Psychology of preferences:}
  ...
  \item Ambiguity aversion (risk preferences when probabilities not objectively known)
  \end{itemize}
...
\item Many non-EU models that try to capture these departures from EU
  \begin{itemize}
  \item Prospect theory, due to Kahneman and Tversky (1979, 1992), is the best known
  \end{itemize}
...
\item The carriers of utility are gains and losses relative to some reference point, not final wealth levels
  ...
  \end{itemize}
\item We are more sensitive to changes in brightness, loudness or temperature rather than to absolute levels of these attributes
  \begin{itemize}
  \item E.g., put two hands in separate hot and cold water, then in one large warm bath
    \begin{itemize}
    \item Hot hand feels colder than the cold hand!
    \end{itemize}
  \end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Turns out that we hate about losses twice as much as we like an equal-sized gain
\item we hate losing
\end{itemize}
\end{column}
...
\begin{itemize}
\item Many of us -- simultaneously -- buy lotteries and insurance
\item Buying lotteries means that people prefer a 1 in 1000 chance of winning \$5000 to a certain \$5
  \begin{itemize}
  \item $(\$5, 1) \prec (\$5000, 0.001)$
  \end{itemize}
\item Buying insurance means that people prefer to pay \$5 than to face a 1 in 1000 chance of losing \$5000
  \begin{itemize}
  \item $(-\$5000, 0.001) \prec (-\$5, 1)$
  \end{itemize}
\item Difficult to explain under Expected Utility: risk aversion and risk seeking at the same time
\item \textbf{Probability weighting:} people over-weight low probability events
\end{itemize}
\end{frame}
...
\item Could come from salience
  ...
  \item How easy we find it to recall the related incident is our mind's simple proxy for how likely the occurrence of the event is
  \end{itemize}
\end{itemize}
\end{column}
...
\item The original version (Kahneman and Tversky, 1979) consider the gamble $(x, p; y, q)$
...
\begin{column}{0.43\textwidth}
\centering
\includegraphics[width=\textwidth]{figures/slide13_pic1.png}
\end{column}
\end{columns}
\end{frame}
...
\item \textbf{Probability weighting}
  \begin{itemize}
  \item Transform probabilities with a weighting function $w(\cdot)$ that over-weights low probabilities
...
\item \textbf{Risk-aversion over gains, risk-seeking over losses}
  ...
  \item But are more willing to hold on to gambles and take risk when they have lost money on it (``convex over losses'')
  \item Inferred from $(\$500, 1) > (\$1000, 1/2; \$0, 1/2)$ and $(-\$500, 1) < (-\$1000, 1/2; \$0, 1/2)$
  \end{itemize}
\end{enumerate}
\end{frame}
...
\item Prefer more volatile stocks in domain of capital losses
\item Prefer less volatile stocks in domain of capital gains
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\centering
...
\begin{itemize}
\item \textbf{Crucial question:} what should the argument of $v(\cdot)$ be?
  \begin{itemize}
...
\item \textbf{The cross-section of stock returns}
  \begin{itemize}
  \item Try to address the high risk -- low alphas and momentum/PEAD puzzle
  \item New prediction: the pricing of skewness
  \item Probability weighting and diminishing sensitivity play the most critical role
  \end{itemize}
\item \textbf{The aggregate stock market}
  ...
  \item Loss aversion plays a key role; but probability weighting also matters
  \end{itemize}
\end{itemize}
\end{frame}
...
\item Prospect theory can explain the disposition effect
  \begin{itemize}
  \item Investors' tendency to hold onto losers and sell winners too quickly
  \end{itemize}
\end{itemize}
\vfill
\centering
...
\begin{frame}{The Cross-section}
\textbf{Barberis and Huang (AER 2008), Stock as Lotteries\ldots}
\begin{itemize}
\item Single period model; a risk-free asset and $J$ risky assets with multivariate Normal payoffs
...
\item Skewness itself is priced, in contrast to EU model where only coskewness with market matters
\end{itemize}
\vspace{0.5em}
...
\begin{itemize}
\item Lose diversification
\item Idiosyncratic skewness outweigh the loss of diversification
\end{itemize}
\vfill
\centering
\includegraphics[width=0.50\textwidth]{figures/22.png}
\hfill\footnotesize Barberis and Huang (2008)
\end{frame}
...
\vfill
\centering
\includegraphics[width=0.50\textwidth]{figures/slide23_pic1.jpg}
\end{frame}
...
\item Low average return of financially distressed stocks and OTC stocks (Eraker and Ready, 2015)
  ...
  \end{itemize}
\item ``Overpricing'' of out-of-money options
  \begin{itemize}
...
\item Low average return on stocks with high idiosyncratic volatility (Ang et al., 2006)
  ...
  \end{itemize}
\item High beta stocks earn lower CAPM alpha
  \begin{itemize}
  \item Beta is positively related to skewness (Bali et al., 2017)
  \end{itemize}
\end{itemize}
\end{column}
\begin{column}{0.43\textwidth}
...
\item Several papers test the model's basic prediction that skewness is priced in the cross-section
  \begin{itemize}
  \item Empirical challenge is that past skewness is not a good measure of expected future skewness
...
\begin{itemize}
\item Bali, Cakici, and Whitelaw (2011) construct a very simple measure of lottery feature for individual stocks
\item \textbf{MAX:} maximum daily return over the past one month
\item Strong negative return predictability of MAX and help resolve the idiosyncratic volatility puzzle
\end{itemize}
\vfill
\centering
\includegraphics[width=0.65\textwidth]{figures/slide26_pic1.png}
\end{frame}

% --- Slide 27: Lottery Preference Evidence ---
\begin{frame}[shrink=5]{Evidence Supporting Investor Lottery Preference}
\begin{itemize}
\item Kumar (2009) shows that demand for lotteries and lottery-type stocks are correlated
...
\item Lottery investment levels are higher in regions with favourable lottery environments
\end{itemize}
\vfill
\centering
\includegraphics[width=0.50\textwidth]{figures/slide27_pic1.png}
\end{frame}

% --- Slide 28: Lottery Preference Evidence, ctd. ---
\begin{frame}[shrink=5]{Evidence Supporting Lottery Preference, ctd.}
\begin{itemize}
...
\item This only works if the security is highly skewed
  \begin{itemize}
  \item Otherwise, would need too undiversified a position in order to add skewness to the portfolio
  \end{itemize}
\item \textbf{Note:}
  \begin{itemize}
  \item An example of how psychology can lead to interesting new predictions
  \item Probability weighting plays a central role
  \item Model generates both a low premium on positively skewed stocks and a high premium on the aggregate market
    \begin{itemize}
...
\item Wang, Yan, and Yu (JFE, 2016):
  \begin{itemize}
  \item Cross-sectional risk-return trade-off is positive among stocks in which investors face prior gains
  \item But becomes negative among stocks in which investors face prior losses
  \end{itemize}
\end{itemize}
\vfill
\centering
\includegraphics[width=0.70\textwidth]{figures/slide30_pic1.jpg}
\end{frame}
...
\textbf{Applications, ctd.}
\begin{itemize}
\item If many investors display disposition effect, there will be underreaction to news
  \begin{itemize}
  \item Grinblatt and Han (2005): Momentum; Frazzini (2006): PEAD
  \end{itemize}
\item When a stock experiences good news and increases in value relative to the purchase price, investors want to sell it to lock in the gain, which depresses its price and leads to higher subsequent return
\item When bad news is released and price goes down relative to the purchase price, disposition investors are reluctant to sell, leading to underreaction and lower future return
\end{itemize}
\vfill
\centering
...
\begin{frame}[shrink=5]{Direct Test of Prospect Theory in Cross-section}
...
\item Stock with high prospect theory value are more appealing to investors
...
% --- Slide 33: BJW 2021 ---
\begin{frame}[shrink=5]{Prospect Theory and Stock Market Anomalies}
...
\vfill
\centering
\includegraphics[width=0.55\textwidth]{figures/slide33_pic1.png}
\end{frame}

% --- Slide 34: BJW 2021, ctd. ---
\begin{frame}[shrink=5]{Prospect Theory and Stock Market Anomalies, ctd.}
\begin{itemize}
...
\vfill
\centering
\includegraphics[width=0.55\textwidth]{figures/slide34_pic1.png}
\end{frame}
...
\item But prospect theory also makes prediction about investor demand for certain assets
\item Han, Sui and Yang (JFE forthcoming) evaluate prospect theory using mutual fund flows, which directly reveal investor preference/demand across assets
\item They document funds whose past returns generate higher prospect theory value attract significantly larger future flows
\end{itemize}
\end{frame}
...
\begin{frame}{The Aggregate Stock Market}
\begin{itemize}
...
\begin{itemize}
\item Prospect theory value of the historical distribution of annual US stock market returns is approximately equal to the prospect theory value of the historical distribution of annual US bond market returns
\item On one hand, the greater volatility of stock market return serves to lower their prospect value relative to that of bond market returns
...
\item Benartzi and Thaler (1995) shows that these two forces cancel out
\item Stock market earns a high average return so that it can be competitive with the bond market in the eyes of prospect theory investors
\end{itemize}
\end{frame}
...
\item Barberis, Huang, and Santos (2001) find that narrow framing is a critical ingredient
  \begin{itemize}
  \item Loss aversion over total wealth fluctuations doesn't produce a significant equity premium
  \item Nor will it generate non-participation in the stock market
  \item Stock market has low correlation with other risks
  \end{itemize}
\end{itemize}
\vspace{0.5em}
\textbf{The role of probability weighting}
\begin{itemize}
\item De Giorgi and Legg (2012) bring probability weighting and they show it can significantly increase the equity premium
  \begin{itemize}
  \item Because the aggregate stock market is negatively skewed: it is subject to occasional large crashes
  \item Under probability weighting, investors overweight this left-tail outcome, which makes the stock market all the less appealing
  \end{itemize}
\end{itemize}
\end{frame}
...
\item Choose between:
  ...
  \end{itemize}
\item The modal participant choices are R1 and B1, suggesting he believes there are both more red and black balls in Urn C than Urn U
\end{itemize}
\end{frame}
...
\item \textbf{Non-participation:}
  \begin{itemize}
  \item The return of the stock market is more ambiguous than the return on a bank deposit or a Treasury Bill (Dow and Werlang, 1992)
  \end{itemize}
